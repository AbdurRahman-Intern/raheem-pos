...
//     return (
//         <AuthenticatedLayout header={<h2 className="text-xl font-semibold text-gray-800">Edit Expense</h2>}>
...
//                             <button type="submit" disabled={processing} className="rounded bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Update Expense</button>
            ...
            header={
                <div>
                    <nav className="mb-1 flex items-center gap-1.5 text-xs text-gray-400">
                        ...
                        <span className="text-gray-500">{t('finance', 'Edit Expense')}</span>
                    </nav>
                    <h2 className="text-xl font-semibold text-gray-800">{t('finance', 'Edit Expense')}</h2>
                </div>
            }
        >
            <Head title={t('finance', 'Edit Expense')} />

            <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
                <form onSubmit={submit}>
                    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                        {/* Amber accent header — echoes the amber "Edit" action color from the table,
                            distinguishing this from the rose "New Expense" create screen */}
                        <div className="flex items-center justify-between gap-3 border-b border-amber-100 bg-amber-50 px-6 py-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-semibold text-amber-800">{t('finance', 'Edit Expense')}</p>
                                    <p className="text-xs text-amber-600">
                                        {t('finance', 'Update the details below and save your changes')}
                                    </p>
                                </div>
                            </div>
                            {isDirty && (
                                <span className="hidden shrink-0 items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700 sm:inline-flex">
                                    <span className="h-1.5 w-1.5 rounded-full bg-amber-500" />
                                    {t('finance', 'Unsaved changes')}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="grid gap-6 p-6 sm:grid-cols-2">
                            ...
                            <div className="sm:col-span-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100"
                                />
                        ...
                        <div className="flex items-center justify-between gap-3 border-t border-gray-100 bg-gray-50 px-6 py-4">
                            <button
                                ...
                                className="text-sm font-semibold text-gray-500 transition hover:text-gray-700 disabled:cursor-not-allowed disabled:opacity-40"
                            >
                                {t('finance', 'Revert changes')}
                            </button>
                            <div className="flex items-center gap-3">
                                <Link
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="inline-flex items-center gap-2 rounded-lg bg-amber-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
                                >
                                    {processing && (
